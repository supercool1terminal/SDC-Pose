...
def compute_combination_mAP(final_results, synset_names, degree_thresholds=[5, 10, 15], shift_thresholds=[0.1, 0.2],
    ...
    for progress, result in tqdm(enumerate(final_results)):
        ...
        for cls_id in range(1, num_classes):
            ...
            gt_match, pred_match, pred_indiced = compute_combination_3d_matches(cls_gt_class_ids, cls_gt_RTs,
                                                                                cls_gt_scales, cls_gt_handle_visibility,
                                                                                synset_names,
                                                                                cls_pred_bboxes, cls_pred_class_ids,
                                                                                cls_pred_scores, cls_pred_RTs,
                                                                                cls_pred_scales,
...
def compute_independent_mAP(final_results, synset_names, degree_thresholds=[360], shift_thresholds=[100],
                            iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=True, logger=None,
                            cat_id=-1):
    ...
    if logger is not None:
        ...
        logger.warning(
            f'5 degree, 2cm per category:{pose_aps[1:, degree_thres_list.index(5), shift_thres_list.index(2)] * 100}')
        logger.warning(
            f'5 degree, 5cm per category:{pose_aps[1:, degree_thres_list.index(5), shift_thres_list.index(5)] * 100}')
        logger.warning(
            f'10 degree, 2cm per category:{pose_aps[1:, degree_thres_list.index(10), shift_thres_list.index(2)] * 100}')
        logger.warning(
            f'10 degree, 5cm per category:{pose_aps[1:, degree_thres_list.index(10), shift_thres_list.index(5)] * 100}')
