...
class Solver(
    ...
    def solve(self):
        while self.epoch <= self.cfg.max_epoch:  # train epoch in side max_epoch
            self.logger.info('\nEpoch {} :'.format(self.epoch))

            # record current debug time
            epoch_start_time = time.time()
            print( f"Epoch {self.epoch} Start time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(epoch_start_time))}")

            end = time.time()
            dict_info_train = self.train()

            # after the end of each epoch, update mean best_iter and print
            # avg_iter, avg_offset_r, avg_offset_t, avg_offset_s, avg_lr = self.model.pose_optimizer.get_avg_iter_and_offsets()

            train_time = time.time() - end

            dict_info = {'train_time(min)': train_time / 60.0}  # print out train time
            ...
            print(f"Epoch {self.epoch} - The model has been saved to: {ckpt_path}")

            prefix = 'Epoch {} - '.format(self.epoch)
            write_info = self.get_logger_info(prefix, dict_info=dict_info)
            # write_info += f"lr: {self.lr_scheduler.get_lr()[0]:.5f}"
            write_info += f"lr: {self.lr_scheduler.get_last_lr()[0]:.5f}"
            self.logger.warning(write_info)
            self.epoch += 1
            epoch_end_time = time.time()
            print(f"Epoch {self.epoch - 1} Completed. Total time consumed:{(epoch_end_time - epoch_start_time) / 60.0:.2f} Minutes, current time:{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
    ...
    def train(self):
        ...
        num_mini_batch_per_epoch = self.cfg.train_dataset.num_mini_batch_per_epoch

        for train_data in data_iter:
            data_time = time.time() - end
            self.optimizer.zero_grad()

            # lr = self.lr_scheduler.get_lr()[0]if self.lr_scheduler is not None else self.cfg.optim.lr
            ...
            self.optimizer.step()  # renew parameters of model, backward propagation
            ...
            # print(f"Epoch {self.epoch} - Mini-batch [{i + 1}/{num_mini_batch_per_epoch}] ({current_progress:.2f}%) completed")
            if (i + 1) % 50 == 0 or (i + 1) == num_mini_batch_per_epoch:
                current_progress = (i + 1) / num_mini_batch_per_epoch * 100 
                print(
                    f"Epoch {self.epoch} - Mini-batch [{i + 1}/{num_mini_batch_per_epoch}] ({current_progress:.2f}%) completed")

            if i % self.per_write == 0:
                self.log_buffer.average(self.per_write)
                prefix = '[{}/{}][{}/{}][{}] Train - '.format(self.epoch, self.cfg.max_epoch, i, iter_lenth, self.iter)
    ...
    def evaluate(self):
        ...
        for i, data in enumerate(self.dataloaders["eval"]):
            with torch.no_grad():  # no need to compute gradients
                ...
                if i % self.per_write == 0:
                    self.log_buffer.average(self.per_write)
                    prefix = '[{}/{}][{}/{}] Test - '.format(self.epoch, self.cfg.max_epoch, i,
    ...
    def step_camera_real(self, train_data, mode):  # handle real dataset, forward and return loss value
        syn_data, real_data = train_data  # Unpacket tuple
        ...
        # handle end_points, exclusive the best_iter due it not is tenor
        for key in end_points:
            if key == 'neighbor_cluster_info':
                syn_data[key] = [x[:b1] for x in end_points[key]] if end_points[key] else []
                real_data[key] = [x[b1:] for x in end_points[key]] if end_points[key] else []
            elif key != 'best_iter':  # best_iter is Scalar quantity
                syn_data[key] = end_points[key][0:b1]
                real_data[key] = end_points[key][b1:]
            else:
                syn_data[key] = end_points[key]  # directly assign scalar
        ...
        # !!! append optimizer iteration to logs
        # dict_info['best_iter'] = end_points['best_iter']

        loss_all = (loss_dict_syn['loss_all'] * b1 + loss_dict_real['loss_all'] * b2) / (b1 + b2)

        if mode == 'train':
            # dict_info['lr'] = self.lr_scheduler.get_lr()[0]
            dict_info['learning_rate'] = self.lr_scheduler.get_last_lr()[0]

        return loss_all, dict_info

    def step_camera(self, train_data, mode):  # handle synthetic dataset
        syn_data = train_data
...
def test_func(model, dataloder, save_path):  
    ...
    with tqdm(total=len(dataloder)) as t:
        for i, data in enumerate(dataloder):
            ...
            # print(f"Batch {i}, pts shape: {inputs['pts'].shape}, choose shape: {inputs['choose'].shape}")
            ...
            # print(f"pred_translation: {pred_translation.shape}, pred_size: {pred_size.shape}, pred_rotation: {pred_rotation.shape}")
            ...
            pred_scales = pred_size  # no normalization
            ...
            target_dir = os.path.join(save_path, path.split('/')[-1])
            target_dir = os.path.dirname(target_dir)  # Get the directory path
