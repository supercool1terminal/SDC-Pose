...
# define deeply_learn network structure employ in convolutional neural network and full connected neural network
class SharedMLP(nn.Sequential): #a shared MLP
    def __init__(
        self,
        args,
        bn=False,
        ...
        for i in range(len(args) - 1):   #through circle for add conv2d
            self.add_module(
                name + "layer{}".format(i),
                Conv2d(
                    args[i],
...
class _BNBase(nn.Sequential):  #define a Batch Normalization, accerlate the training process and improve the performance, avoid gradient vanishing and exploding
...
class _ConvBase(nn.Sequential): #define a Convolution operation, include Convolution, Batch Normalization and Activation function
    def __init__(
        self,
        in_size,
        out_size,
...
class FC(nn.Sequential): # full-connected layer
    def __init__(
        self,
        in_size,
        out_size,
...
def group_model_params(model, **kwargs): #will model parameters into two groups, one with weight decay and one without weight decay
...
def set_bn_momentum_default(bn_momentum): #return a function to set bn momentum
    def fn(m):
...
class BNMomentumScheduler(object): #activate adjustment of bn momentum
