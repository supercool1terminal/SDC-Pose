...
def get_parser(): 
    ...
    parser.add_argument('--mask_label', action='store_true', default=True,
...
if __name__ == "__main__":
    ...
    if not cfg.only_eval:  
        ...
        model = model.cuda() #Instantiate the model and move it to the GPU;
        ...
        dataloder = torch.utils.data.DataLoader(
                ...
                shuffle=True,
                drop_last=False
            ) #Create the TestDataset and DataLoader to load the test dataset
        test_func(model, dataloder, save_path) #Execute test

    # evaluate(os.path.join(save_path, 'data', 'segmentation_results', 'CAMERA25'), logger, cat_id=cfg.cat_id)
    evaluate(os.path.join(save_path, 'data', 'segmentation_results', 'REAL275'), logger, cat_id=cfg.cat_id)
