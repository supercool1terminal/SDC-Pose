...
def PoseDis(r1, t1, s1, r2, t2, s2):
    ...
    # dis_r = torch.norm(r1 - r2, p='fro', dim=(1, 2)).mean()
    # dis_t = torch.norm(t1 - t2, dim=1).mean()
    # dis_s = torch.norm(s1 - s2, dim=1).mean()
    # ortho_loss = torch.abs(torch.det(r1) - 1).mean()  # 惩罚行列式偏离 1

    return dis_r + dis_t + dis_s

def UniChamferDis(p1, p2):  #single direct chamfer distance, not symmetric
    '''
    p1: b, n1, 3
    p2: b, n2, 3
    '''
