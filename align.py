...
def solve_Umeyama(nocs, pts):
    """_summary_

    Args:
        nocs (_type_): (b, kpt_num, 3)
        pts (_type_): (b, kpt_num, 3)
    """
    num_instances = nocs.shape[0]
    scales = np.zeros(num_instances)
    rotations = np.zeros((num_instances, 3, 3))
    translations = np.zeros((num_instances, 3))
    
    for idx in range(num_instances):
        s, R, T, outtransform = estimateSimilarityTransform(nocs[idx], pts[idx], False)
        scales[idx] = s
        rotations[idx, :, :] = R
        translations[idx, :] = T
